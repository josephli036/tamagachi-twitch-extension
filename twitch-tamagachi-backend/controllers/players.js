...
playersRouter.post('/', async (req,res) => {
    ...
    */ 
    ...
    const playerQuery = 'SELECT * FROM players WHERE opaque_user_id = $1 and channel_id = $2'
    let player = (await client.query(playerQuery, [currentOpaqueUserId, channelId])).rows
    
    if (player.length === 0) {
        console.log("in here?")
        const currentTime = new Date()
        const newPlayer = {channel_id: channelId, opaque_user_id: currentOpaqueUserId, last_updated: currentTime, total_points: 1, points_to_spend:1, attack_stat:0, jump_stat:0, shield_stat:0, focus_stat:0}
        ...
        if (currentTime >= pointThresholdTime) {
            const updatePlayerQuery = 'UPDATE players SET total_points = $1, points_to_spend =$2 WHERE opaque_user_id = $3 and channel_id = $4'
            await client.query(updatePlayerQuery, [Number(player.total_points)+1, Number(player.points_to_spend)+1, currentOpaqueUserId, channelId])
